...
namespace GoShared
{
  ...
  public class LocationManager : BaseLocationManager
  {
    ...
    // 是否有按下按鈕
    private bool isLeftBtnPress = false;
    private bool isRightBtnPress = false;
    private bool isUpBtnPress = false;
    private bool isDownBtnPress = false;
    ...
    public void onPressBtn(PressType pressType)
    {
      switch (pressType)
      {
        case PressType.Left:
    ...
    public void onBottomBtn()
    {
      isDownBtnPress = true;
    }

    private void onMoveBtn(Vector3 v3)
    {
      Vector3 current = currentLocation.convertCoordinateToVector();
      Vector3 v = current + v3;
      currentLocation = Coordinates.convertVectorToCoordinates(v);
      if (onLocationChanged != null)
      {
        onLocationChanged.Invoke(currentLocation);
